...
class BeliefUnit(nn.Module):
    ...
    def forward(self, h_t, c_t, u_t):
        ...
        *
        ...
        return h_t_next.view(nb_ag, batch_size, -1), \
               c_t_next.view(nb_ag, batch_size, -1)
...
class ActionUnit(nn.Module):
    ...
    def forward(self, h_caret_t, c_caret_t, u_t):
        ...
        h_caret_t, c_caret_t, u_t = \
            h_caret_t.flatten(0, 1), \
            c_caret_t.flatten(0, 1), \
            u_t.flatten(0, 1)

        h_caret_t_next, c_caret_t_next = \
            self.lstm(u_t, (h_caret_t, c_caret_t))

        return h_caret_t_next.view(nb_ag, batch_size, -1), \
               c_caret_t_next.view(nb_ag, batch_size, -1)
